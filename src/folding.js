...
function scalarField(theta, phi, layer, d) {
	...
	return (
		1.0 + m1 * (0.18 + d.fold * 0.28) + m2 * (0.14 + d.warp * 0.12) + m3 * 0.12 + m4 * 0.08 + spikeField * d.spike * 0.55 + asym * 0.38
	);
}
...
function buildLayerGeometryData(d, layerIndex, meshConfig) {
	...
	for (let y = 0; y <= meshConfig.vSeg; y++) {
		...
		for (let x = 0; x <= meshConfig.uSeg; x++) {
			...
			const spikeNeedle = Math.pow(
				Math.abs(Math.sin(theta * (d.order + 3.0) - phi * 6.0 + layerIndex)),
				10.0 - Math.min(d.spike * 3.0, 5.5)
			);
			...
			positions.push(
				sx * r * anisotropy * meshConfig.xzScale,
				sy * r * (1.0 + d.fold * meshConfig.yFoldScale),
				sz * r * anisotropy * meshConfig.xzScale
			);
		}
	}
